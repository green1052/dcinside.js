...
`Unable to write article: 잠시후 다시 이용해주세요.`는 요청 형식은 통과했지만 DCInside 서버가 작성을 거절했을 때 자주 나오는 응답입니다. 라이브러리 입력 오류보다는 서버의 세션/작성
정책인 경우가 많습니다.

- 제목/본문이 너무 짧거나 반복적일 때
- 같은 IP나 세션에서 짧은 시간 안에 여러 번 작성했을 때
- 갤러리의 비회원, 로그인 세션, 이미지, 말머리 정책에 걸렸을 때
- 서버 측 임시 제한이 걸렸을 때

제목과 본문을 충분히 구체적으로 바꾸고, 필요한 말머리를 지정한 뒤 잠시 후 재시도하세요.