...
작업 시도 (write/upvote/login)
  ...
  │   └─ downloadCaptchaImage({ url }) → result.bytes
  │
  └─ 사용자 입력 코드로 재시도
      └─ captcha: { code, dccode }
```
...
```ts
import {CaptchaRequiredError} from "@green-1052/dcinside.js";
...
```ts
import {createCaptchaChallenge} from "@green-1052/dcinside.js";
...
```ts
import {downloadCaptchaImage} from "@green-1052/dcinside.js";
...
```ts
import {LoginCaptchaRequiredError, createCaptchaChallenge, downloadCaptchaImage} from "@green-1052/dcinside.js";
import {writeFileSync} from "node:fs";
